...
class DataLoader:
    ...
    def preprocess_data(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        # Feature engineering
        processed_df['RoomsPerHousehold'] = processed_df['AveRooms'] / processed_df['AveOccup']
        processed_df['BedroomsPerRoom'] = processed_df['AveBedrms'] / processed_df['AveRooms']
        processed_df['PopulationPerHousehold'] = processed_df['Population'] / processed_df['AveOccup']
        ...
        # Filter out outliers
        outlier_condition = ~((processed_df < (Q1 - 1.5 * IQR)) | 
                             (processed_df > (Q3 + 1.5 * IQR))).any(axis=1)
    ...
    def load_processed_data(self) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
        ...
        data_files = ['X_train.npy', 'X_val.npy', 'X_test.npy', 'y_train.npy', 'y_val.npy', 'y_test.npy']
        data = []
...
def main():
    ...
    # Split and scale
    X_train, X_val, X_test, y_train, y_val, y_test = loader.split_and_scale_data(processed_data)
