...
from api.schemas import (
    ...
    HealthResponse, ModelMetrics, ErrorResponse, HousingFeatures
)
...
# Prometheus metrics
PREDICTION_COUNT = Counter('prediction_requests_total', 'Total prediction requests', ['model_version', 'endpoint'])
PREDICTION_LATENCY = Histogram('prediction_duration_seconds', 'Prediction latency', ['model_version', 'endpoint'])
ERROR_COUNT = Counter('prediction_errors_total', 'Total prediction errors', ['error_type'])
...
class ModelService:
    ...
    def load_model(self, model_path: str = "artifacts/best_model.joblib") -> None:
        ...
        try:
            if os.path.exists(model_path):
                ...
                logger.warning("Model file not found, using fallback", model_path=model_path)
    ...
    def preprocess_features(self, features: HousingFeatures) -> np.ndarray:
        ...
        feature_values.extend([rooms_per_household, bedrooms_per_room, population_per_household])
...
async def log_request(request_data: Dict[str, Any], response_data: Dict[str, Any], endpoint: str):
